...
const CustomNode = ({ data }) => {
  ...
  const { importanceTier, themeColor, themeColorGlow, isFaded, showLabels, enableHighlight } = data;
...
function GraphInner({ notes, links, searchQuery = '', selectedTag = '' }) {
  ...
  const { nodes: layoutedNodes, edges: layoutedEdges } = useMemo(() => {
    ...
    // 1. HARD FILTERING: Only include nodes that match search/tag
    ...
    const filteredNoteIds = new Set(filteredNotes.map(n => String(n.id)));

    // 2. HARD FILTERING: Only include edges where both nodes are visible
    const filteredLinks = links.filter(l => 
      filteredNoteIds.has(String(l.sourceId)) && filteredNoteIds.has(String(l.targetId))
    );
    ...
    const maxConnections = Math.max(1, ...filteredNotes.map(n => 
      filteredLinks.filter(l => l.sourceId === n.id || l.targetId === n.id).length
    ));

    const rawNodes = filteredNotes.map((note) => {
      const connectionsCount = filteredLinks.filter(l => l.sourceId === note.id || l.targetId === note.id).length;
    ...
    const rawEdges = filteredLinks.map(link => {
      let isFaded = false;
      if (graphMode === 'focus' && focusedNodeId) {
  ...
  }, [notes, links, focusedNodeId, graphMode, pathSourceId, pathTargetId, searchQuery, selectedTag, showLabels, enableHighlight]);
...
export default function GraphComponent({ notes, links, searchQuery, selectedTag }) {
  return (
    <div className="w-full h-full relative flex-1 min-h-[500px]">
      <ReactFlowProvider>
        <GraphInner notes={notes} links={links} searchQuery={searchQuery} selectedTag={selectedTag} />
      </ReactFlowProvider>
    </div>
  );
}
